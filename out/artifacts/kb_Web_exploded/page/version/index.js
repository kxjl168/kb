...
function init() {
	...
	$scope.$apply(function () {
		...
		// $("#s_type").find("option[text='Linux']").attr("selected",true);
		setTimeout(function () {
		...
		$scope.del = function (item) {
			...
			$("#btnconfirm").one(
				...
				function () {
					...
					SZUMWS(http + "version/versionInfoDel.action", JSON
		...
		kvalidate.resetForm("#fm");
			...
			if (item != null) {
				...
				$scope.recordid = item.recordid;
				$scope.effectDate = item.effectdateStr;

				$scope.s_desc = item.annotation;
				$scope.s_vnum = item.versionNumStr;
				$scope.url = item.upgradepackagePath;

				$scope.md5 = item.md5;
				$scope.s_type = item.clientType;
				$("#s_type").val($scope.s_type);
				...
				$scope.oldname = item.upgradepackageName;
		...
		$scope.doupdate = function () {
			...
			var url = "version/versionInfoUpdate.action";
			if ($scope.recordid == "")
				url = "version/versionInfoAdd.action";

			var obj = {};

			obj.effectdateStr = $scope.effectDate;
			obj.upgradepackageName = $("#oldname").val();
			obj.isforceupgrade = "0";
			obj.annotation = $scope.s_desc;
			obj.versionNum = $scope.s_vnum;
			obj.upgradepackagePath = $("#url").val();
			obj.md5 = $("#md5").val();
			obj.clientType = $("#s_type").val();
			obj.recordid = $scope.recordid;

			SZUMWS(http + url, JSON.stringify(obj), function succsess(json) {
		...
		$scope.getcompayList = function (id, fucOnFinished, clear) {
			...
			SZUMWS(http + "company/getInfoList.action", JSON.stringify(obj),
		...
		$scope.getList = function (id, fucOnFinished, clear) {
			...
			obj.packageName = $scope.packageName;// "12345678";
			obj.clientType = 0;// $scope.clientType;// "12345678";

			obj.page = $scope.page;// 1;// "12345678";
			obj.rows = $scope.rows;// 10;// "12345678";
			SZUMWS(http + "version/getVersionInfoList.action", JSON
				.stringify(obj), function succsess(json) {
		...
		$scope.Search = function () {
			...
			if (devMode == 0) {
				...
				$.each($scope.respData2.GuideList, function (i, e) {
					if (e.Title.indexOf($scope.Key) > 0
						|| e.Title2.indexOf($scope.Key) > 0)
						$scope.guideList.push(e);
				});
