...
function test(){
	...
	var url = getRPath()+"/manager/calthings/test";
...
$(function() {
	...
	initmenu($("#menuul"), "manager/calthings/manager/");
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/calthings/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/calthings/calthingsList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/calthings/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/calthings/delete";
