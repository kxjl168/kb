...
$(function() {
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/myip/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/myip/myipList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/myip/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/myip/delete";
