...
function test(){
	...
	var url = getRPath()+"/manager/replayinfo/test";
...
$(function() {
	...
	initmenu($("#menuul"), "manager/replayinfo/manager/");
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/replayinfo/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/replayinfo/replayinfoList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
			...
			},
			 {
				...
				valign : 'middle',
				 ...
				 formatter: function (value, row, index) {
		            	 ...
		            	 return unescape(value)+"("+unescape(row.userBlog)+")";
				...
				,
				 ...
				 formatter: function (value, row, index) {
		             if(value==0)
		            	 ...
		            	 return "<span class='green'>可见</span>";
		         }
...
function modifyAndDeleteButton_item(value, row, index) {
	var html='<div class="">';
		...
		if(row.state==0)
			html+= '<button id = "update" type = "button" class = "btn btn-info"><i class="glyphicon glyphicon-check">通过</i> </button>&nbsp;'
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		...
		cconfirm2('审核通过？',function(){
			$.ajax({
				type : "post",
				url :getRPath()+ '/replay/updateState.action',
				data : {
					data : JSON.stringify({'id':row.recordid,'state':'1'}),
				},
				async : false,
				dataType : "json",
				success : function(response) {
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/replayinfo/delete";
