	...
	$("#btnSubmit")
			.click(
					function() {
						...
						var url= basePath+"/privilege/permission/saveOrUpdate.action";
						if(action=="new")
							url= basePath+"/privilege/permission/save.action";
						else
							url= basePath+"/privilege/permission/update.action";
...
function changeParentList(successCallback){
	...
	$.ajax({
		type : "post",
		url : basePath+"/privilege/permission/list.action",
		data : data,
		async : false,
		dataType : "json",
		success : function(data) {
			...
			$.each(lst,function(index,item){
				if(index==0)
				html+=	' <option value="'+item.menuId+'"  selected="selected" >'+item.menuName+'</option> ';
				else
...
function groupTable() {
	...
	$('#table_list').bootstrapTable({
		url :  basePath+'/privilege/permission/list.action', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
...
window.groupEvents = {
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url =  basePath+"/privilege/permission/delete.action";
		//cconfirm(msg);
	...
	"click #update" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url =  basePath+"/privilege/permission/load.action";
		...
		/*if (flag) {*/
			$.ajax({
				...
				success : function(data) {
					...
					changeParentList(function(){
						$('#menuParentid').val(data.menuParentid);// get(0).selectedIndex = 1;
					})
					
					

					$("#menuUrl").val(data.menuUrl);
					$("#percode").val(data.menuId);
					
					//$("#parentid").val(data.parentid);
					
					$("#menuOrderid").val(data.menuOrderid);
					$("#faction").html("修改");
					$("#available").find(
...
function loadMenuTree(role_id)
{
	...
	$.ajax({
		...
		dataType : "json",
		url :  basePath+"/privilege/permission/getMenuTreeSecond.action",// 请求的action路径
