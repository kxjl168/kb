...
function test(){
	...
	var url = getRPath()+"/manager/linkrelation/test";
...
$(function() {
	...
	initmenu($("#menuul"), "manager/linkrelation/manager/");
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/linkrelation/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/linkrelation/linkrelationList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
...
window.PersonnelInformationEvents_item = {
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/linkrelation/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/linkrelation/delete";
