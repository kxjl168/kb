...
function test(){
	...
	var url = getRPath()+"/manager/searchkeys/test";
...
$(function() {
	...
	initmenu($("#menuul"), "manager/searchkeys/manager/");
	...
	$("#btnSubmit_item").click(function() {
		...
		var url = getRPath()+"/manager/searchkeys/saveOrUpdate";
...
function InitQuery_item() {
	...
	$('#table_list_item').bootstrapTable({
		url : getRPath()+'/manager/searchkeys/searchkeysList', // 请求后台的URL（*）
		method : 'post', // 请求方式（*）
		...
		queryParams : function queryParams(params) { // 设置查询参数
			var param = {
				...
				gkey : $("#q_gkey").val(),
				useTime : $("#q_useTime").val(),
				email : $("#q_email").val(),
				context : $("#q_context").val(),
			...
			},
			 {
				...
				valign : 'middle',
				 formatter: function (value, row, index) {
			            // return new Date(value).Format("yyyy-MM-dd hh:mm:ss");
						if(value=="1")
							return "<span class='text-success fa fa-check-square-o'>"+  row.updateDate.substr(0,row.updateDate.length-2)+"</span>";
						else
...
function modifyAndDeleteButton_item(value, row, index) {
	return [ '<div class="">'
			...
			+ '<button id = "done" type = "button" class = "btn btn-success"><i class="glyphicon glyphicon-envelope">回复</i> </button>&nbsp;'
...
window.PersonnelInformationEvents_item = {
		"click #done" : function(e, value, row, index) {
			$.ajax({
				type : "post",
				url :getRPath()+ '/manager/searchkeys/done',
				data : {
					id : row.id
				},
				async : false,
	...
	"click #update" : function(e, value, row, index) {
		$.ajax({
			type : "post",
			url :getRPath()+ '/manager/searchkeys/load',
			data : {
				id : row.id
			},
			async : false,
	...
	"click #delete" : function(e, value, row, index) {
		var msg = "您真的确定要删除吗？";
		var url = getRPath()+"/manager/searchkeys/delete";
