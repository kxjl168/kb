...
CKEDITOR.dialog.add('ckawesomeDialog', function( editor ) {
	     ...
	     $.ajax({
	        ...
	        success: function(response) {
	        	...
	        	$.each(styles, function( index, value ) {
	        		...
	        		html+="<option vlaue='"+value+"'>"+value+"</option>";
	        	})
	        	
	        	var selectbx = $('#' + selectList.getInputElement().getAttribute('id'));
	        	selectbx.append(html);
	...
	function formatSelectionCKAwesome (icon) {
	  if (!icon.id) { return icon.text; }
	  var text = icon.text.replace(/fa-|\.|\-/gi, " ");
	  var icon = $('<span class="ckawesome_select_options"><i class="fa ' + icon.element.value + ' fa-fw"></i> ' + text + "</span>");
	  return icon;
	};
	
	function preview(){
		  var cka = document.createElement( 'span' );

		 var dlg= CKEDITOR.dialog.getCurrent();
		  
          var cka_size = dlg.getValueOf( 'options', 'textsize' );
          var cka_color = dlg.getValueOf( 'options', 'fontcolor' );
          var cka_class = "fa fa-fw " + dlg.getValueOf( 'options', 'ckawesomebox' );
                ...
                elements: [
                    {
					    ...
					    onLoad: function () {
						   	...
						   	try {
						   		$(selectbx).select2({ width: "100%", templateResult: formatCKAwesome, templateSelection: formatSelectionCKAwesome});
						   		
						   		$(selectbx).on('select2:select', function(e) {
						   			preview();
						   		});
                    ...
                    
                    {
                    	 ...
                    	 type: "html",
                         padding: 0,
                         html: '<div id="myDiv">Preview: <div id="previewDiv">Another div.</div></div>',
                    ...
                    
                    {
                        ...
                        onShow: function(){
					    	var selectbx = $('#' + this.getInputElement().getAttribute('id'));  
					    	$(selectbx).val('15').trigger('change') ;
					    },
					    onChange:function(){
					    	preview();
					    }
