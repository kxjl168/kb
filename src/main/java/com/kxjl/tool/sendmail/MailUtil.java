...
public class MailUtil {
	...
	public static boolean sendMail(String recvmail, String title, String message) {
		...
		mail.setPassword(ConfigReader.getInstance().getProperty("MAILSENDERPASSWORD")); // 发件人邮箱的登录密码
	 ...
	 * @date 2020年1月16日
	 */
	public static boolean sendMail2(String recvmail, String title, String message, String nickname, String action,
			String bloglink, String blogtitle, String replayData, String replayLink) {
		Mail mail = new Mail();
		mail.setHost(ConfigReader.getInstance().getProperty("MAILHOSTSERVER")); // 设置邮件服务器,如果不用163的,自己找找看相关的
		mail.setSender(ConfigReader.getInstance().getProperty("MAILSENDER"));
		mail.setReceiver(recvmail); // 接收人
		...
		mail.setMessage(message);
		return new MailUtil().sendTemplate(mail, nickname, action, bloglink, blogtitle, replayData, replayLink);
	}
	 ...
	 * @date 2020年1月16日
	 */
	public static boolean sendMailCode(String recvmail, String title, String message) {
		Mail mail = new Mail();
		mail.setHost(ConfigReader.getInstance().getProperty("MAILHOSTSERVER")); // 设置邮件服务器,如果不用163的,自己找找看相关的
		mail.setSender(ConfigReader.getInstance().getProperty("MAILSENDER"));
		mail.setReceiver(recvmail); // 接收人
		...
		mail.setMessage(message);
		return new MailUtil().sendTemplateCode(mail,message);
	}

	private static String mailtpStr = "";

	// 模板-名称，模板内容
	private static Map<String, String> mailtpMap = new HashMap<>();
	 ...
	 * @date 2020年1月16日
	 */
	public static String getTempateHtml(String templateName) {
		String str = "";
		try {

			StringBuilder sb = new StringBuilder();
			InputStream in = MailUtil.class.getClassLoader().getResourceAsStream(templateName);// "mailtp.xml");
			InputStreamReader reader = new InputStreamReader(in); // 建立一个输入流对象reader
			BufferedReader br = new BufferedReader(reader); // 建立一个对象，它把文件内容转成计算机能读懂的语言
			String line = "";
			line = br.readLine();
	...
	public String getTmpCode(String tempName, String content) {
		if (mailtpMap.get(tempName) == null || mailtpStr.equals(tempName)) {
			String template = getTempateHtml(tempName);
			mailtpMap.put(tempName, template);
		}
		...
		String template = mailtpMap.get(tempName);

		String mail = template;
		mail = mail.replace("{{context}}", content);
	...
	public boolean sendTemplate(Mail mail, String htmlMsg) {
		// 发送email
		...
		try {
			...
			String SSL_SMTP = ConfigReader.getInstance().getProperty("SSL_SMTP", "false");
			if (SSL_SMTP.equalsIgnoreCase("true")) {

				email.setSSLOnConnect(true); // 是否启用SSL
				System.setProperty("mail.smtp.ssl.enable", "true");

				String SSL_SMTP_PORT = ConfigReader.getInstance().getProperty("SSL_SMTP_PORT", "465");
	 ...
	 * @date 2020年1月16日
	 */
	public boolean sendTemplate(Mail mail, String nickname, String action, String bloglink, String blogtitle,
			String replayData, String replayLink) {

		String htmlmsg = getTmp2(nickname, action, bloglink, blogtitle, replayData, replayLink);

		return sendTemplate(mail, htmlmsg);
	 ...
	 * @date 2020年1月16日
	 */
	public boolean sendTemplateCode(Mail mail,  String content) {

		String htmlmsg = getTmpCode("codetp.xml",content);
	...
	public boolean send(Mail mail) {
		...
		try {
			...
			try {
				...
				/*
				 * url = new URL("http://www.apache.org/images/asf_logo_wide.gif"); String cid =
				 * email.embed(url, "Apache logo.gif");
				 * 
				 * // set the html message
				 * email.setHtmlMsg("<html>The apache logo - <img src=\"cid:" + cid +
				 * "\">  "+msg+" <script>alert(1);<script> </html>");
				 */
