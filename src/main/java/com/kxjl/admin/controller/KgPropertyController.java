 ...
 */
@RestController
@RequestMapping("/kg/kg-property")
public class KgPropertyController {  
    
    Logger logger=LoggerFactory.getLogger(KgPropertyController.class);
	 ...
	 */
	@PostMapping("/add")
	public WZResponseEntity<?> add(@CurrentUser LoginUser user, KgProperty kgProperty) {
		logger.info("kg add kgProperty:{}", kgProperty);
		
		WZResponseEntity<KgProperty> returnrst=	 new WZResponseEntity<KgProperty>();
		
		returnrst= (WZResponseEntity<KgProperty>)kgPropertyService.add(user,kgProperty);
		
		//返回带rule 规则的数据.
		KgProperty data= kgPropertyService.getOne(kgProperty.getId(),Constants.DEFULT_VERSION);
		
		returnrst.setBody(data);
		return returnrst;
	}
	 ...
	 */
	@PutMapping("/modify")
	public WZResponseEntity<?> modify(@CurrentUser LoginUser user,KgProperty kgProperty) {
		logger.info("kg modify kgProperty:{}", kgProperty);
		return kgPropertyService.modify(user,kgProperty);
	}
	...
	public WZResponseEntity<?> delete(@CurrentUser LoginUser user,@PathVariable String id) {
		logger.info("kg delete kgProperty:{}", id);
		return new WZResponseEntity<>(kgPropertyService.delete(user,id,Constants.DEFULT_VERSION));
	}
	 ...
	 */
	@GetMapping("/get/{id}")
	public WZResponseEntity<KgProperty> getOne(@PathVariable String id) {
		logger.info("kg get kgProperty:{}", id);
		KgProperty result = kgPropertyService.getOne(id,Constants.DEFULT_VERSION);
		if (result == null) {
	...
	@RequestMapping("/list")
	@ResponseBody
	public WZResponseEntity<List<KgProperty>> getListByPage(@CurrentUser LoginUser user, KgProperty query, Pagination pageCondition) {
		//System.out.println(pageCondition.getPageSize());
		Page<KgProperty> result = kgPropertyService.selectList(user,query, pageCondition);
		WZResponseEntity<List<KgProperty>> rst = new WZResponseEntity<>(result.getResults());
	...
	@ResponseBody
	public WZResponseEntity<?> audit(@CurrentUser LoginUser user, KgProperty query,String editDataStr) {
		
		 KgEditData editdata=JSONObject.parseObject(editDataStr,KgEditData.class);
		
		return kgPropertyService.audit(user,query, editdata);
