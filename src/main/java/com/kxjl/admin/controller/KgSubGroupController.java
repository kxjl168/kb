 ...
 */
@RestController
@RequestMapping("/kg/kg-sub-group")
public class KgSubGroupController {  
    
    Logger logger=LoggerFactory.getLogger(KgSubGroupController.class);
	 ...
	 */
	@PostMapping("/add")
	public WZResponseEntity<?> add(@CurrentUser LoginUser user, KgSubGroup kgSubGroup) {
		logger.info("kg add kgSubGroup:{}", kgSubGroup);
		return new WZResponseEntity<>(kgSubGroupService.add(user,kgSubGroup));
	}
	 ...
	 */
	@PutMapping("/modify")
	public WZResponseEntity<?> modify(@CurrentUser LoginUser user, KgSubGroup kgSubGroup) {
		logger.info("kg modify kgSubGroup:{}", kgSubGroup);
		return new WZResponseEntity<>(kgSubGroupService.modify(user,kgSubGroup));
	}
	...
	public WZResponseEntity<?> delete(@CurrentUser LoginUser user,@PathVariable String id) {
		...
		return new WZResponseEntity<>(kgSubGroupService.delete(user,id,Constants.DEFULT_VERSION));
	}
	 ...
	 */
	@GetMapping("/get/{id}")
	public WZResponseEntity<KgSubGroup> getOne(@PathVariable String id) {
		logger.info("kg get kgSubGroup:{}", id);
		KgSubGroup result = kgSubGroupService.getOne(id,Constants.DEFULT_VERSION);
		if (result == null) {
	...
	@RequestMapping("/list")
	@ResponseBody
	public WZResponseEntity<List<KgSubGroup>> getListByPage(@CurrentUser LoginUser user, KgSubGroup query, Pagination pageCondition) {
		//System.out.println(pageCondition.getPageSize());
		Page<KgSubGroup> result = kgSubGroupService.selectList(user,query, pageCondition);
		WZResponseEntity<List<KgSubGroup>> rst = new WZResponseEntity<>(result.getResults());
	...
	@ResponseBody
	public WZResponseEntity<?> audit(@CurrentUser LoginUser user, KgSubGroup query,String editDataStr) {
		
		 KgEditData editdata=JSONObject.parseObject(editDataStr,KgEditData.class);
		
		return kgSubGroupService.audit(user,query, editdata);
