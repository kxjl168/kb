 ...
 */
@RestController
@RequestMapping("/kg/kg-edit-class")
public class KgEditClassController {  
    
    Logger logger=LoggerFactory.getLogger(KgEditClassController.class);
	 ...
	 */
	@PostMapping("/add")
	public WZResponseEntity<?> add(@CurrentUser LoginUser user,@RequestBody KgEditClass kgSubGroup) {
		logger.info("kg add kgSubGroup:{}", kgSubGroup);
		return new WZResponseEntity<>(kgEditClassService.add(user,kgSubGroup));
	}
	 ...
	 */
	@PutMapping("/modify")
	public WZResponseEntity<?> modify(@CurrentUser LoginUser user,@RequestBody KgEditClass kgSubGroup) {
		logger.info("kg modify kgSubGroup:{}", kgSubGroup);
		return new WZResponseEntity<>(kgEditClassService.modify(user,kgSubGroup));
	}
	...
	public WZResponseEntity<?> delete(@CurrentUser LoginUser user,@PathVariable String id) {
		...
		return new WZResponseEntity<>(kgEditClassService.delete(user,id));
	}
	 ...
	 */
	@GetMapping("/get/{id}")
	public WZResponseEntity<KgEditClass> getOne(@PathVariable String id) {
		logger.info("kg get kgSubGroup:{}", id);
		KgEditClass result = kgEditClassService.getOne(id);
		if (result == null) {
