 ...
 */
@RestController
@RequestMapping("/kg/kg-edit-property")
public class KgEditPropertyController {  
    
    Logger logger=LoggerFactory.getLogger(KgEditPropertyController.class);
	 ...
	 */
	@PostMapping("/add")
	public WZResponseEntity<?> add(@CurrentUser LoginUser user, KgEditProperty kgSubGroup) {
		logger.info("kg add kgSubGroup:{}", kgSubGroup);
		return new WZResponseEntity<>(kgEditPropertyService.add(user,kgSubGroup));
	}
	 ...
	 */
	@PutMapping("/modify")
	public WZResponseEntity<?> modify(@CurrentUser LoginUser user, KgEditProperty kgSubGroup) {
		logger.info("kg modify kgSubGroup:{}", kgSubGroup);
		return new WZResponseEntity<>(kgEditPropertyService.modify(user,kgSubGroup));
	}
	...
	public WZResponseEntity<?> delete(@CurrentUser LoginUser user,@PathVariable String id) {
		...
		return new WZResponseEntity<>(kgEditPropertyService.delete(user,id));
	}
	 ...
	 */
	@GetMapping("/get/{id}")
	public WZResponseEntity<KgEditProperty> getOne(@PathVariable String id) {
		logger.info("kg get kgSubGroup:{}", id);
		KgEditProperty result = kgEditPropertyService.getOne(id);
		if (result == null) {
