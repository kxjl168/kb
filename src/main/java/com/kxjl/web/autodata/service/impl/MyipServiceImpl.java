...
public class MyipServiceImpl implements MyipService {
	...
	public JSONObject saveMyip(Myip item) {
	...
	@Override
	@Transactional(rollbackFor = Exception.class)
	public JSONObject updateMyip(Myip item) {
		JSONObject rtn = new JSONObject();
	...
	@Override
	public List<Myip> selectMyipList(Myip item) {
		List<Myip> itemList = new ArrayList<>();
		try {
	...
	@Transactional(rollbackFor = Exception.class)
	@Override
	public int deleteMyip(Myip item) {
		int result = 0;
		try {
	...
	@Override
	public Myip selectMyipById(String id) {
		Myip data = null;

		Myip query = new Myip();
		query.setId(id);

		List<Myip> datas = selectMyipList(query);
